...
import { ArrowLeft, Clock, Building2, Calendar, AlertTriangle, CheckCircle, User } from 'lucide-react';
...
export default function TrainingDetailPage() {
  ...
  return (
    <div className="p-8">
      {/* Back button */}
      <Link href="/training" className="inline-flex items-center gap-2 text-slate-600 hover:text-slate-900 mb-6">
      ...
      <div className="bg-white rounded-xl shadow-sm p-6 mb-6">
        ...
        <div className="flex flex-wrap gap-6 text-sm">
          ...
          <div className="flex items-center gap-2 text-slate-600">
            ...
            <span>Created {format(parseISO(training.createdDate), 'MMM d, yyyy')}</span>
          </div>
          <div className="flex items-center gap-2 text-slate-600">
            <Calendar className="w-4 h-4" />
            <span>Updated {format(parseISO(training.lastUpdated), 'MMM d, yyyy')}</span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2">
          <div className="bg-white rounded-xl shadow-sm">
            <div className="p-6 border-b border-slate-100">
              <h2 className="text-lg font-semibold text-slate-900">Training Content</h2>
            </div>
            <div className="p-6">
              <div className="prose prose-slate max-w-none">
                <p className="text-slate-600 whitespace-pre-line">{training.content}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="lg:col-span-1">
          <div className="bg-white rounded-xl shadow-sm">
            <div className="p-6 border-b border-slate-100">
              <h2 className="text-lg font-semibold text-slate-900">Assignment Tracking</h2>
            </div>
            <div className="p-6">
              <div className="space-y-4">
                <div className="flex items-center justify-between p-3 bg-slate-50 rounded-lg">
                  <span className="text-sm text-slate-600">Not Started</span>
                  <span className="font-semibold text-slate-900">{stats.notStarted}</span>
                </div>
                ...
                <div className="flex items-center justify-between p-3 bg-yellow-50 rounded-lg">
                  ...
                  <span className="font-semibold text-yellow-700">{stats.atRisk}</span>
                </div>
                <div className="flex items-center justify-between p-3 bg-green-50 rounded-lg">
                  <span className="text-sm text-green-600">Complete</span>
                  <span className="font-semibold text-green-700">{stats.complete}</span>
                </div>
              ...
              {stats.total > 0 && (
                <div className="mt-6">
                  ...
                  <p className="text-sm text-slate-500 mt-2 text-center">
                    {Math.round((stats.complete / stats.total) * 100)}% completion rate
                  </p>
                </div>
              )}
            </div>
            ...
            {assignments.map(assignment => (
              ...
              >
                ...
                <div className="flex-1">
                  <h3 className="font-medium text-slate-900">{assignment.employee.name}</h3>
                  <p className="text-sm text-slate-500">{assignment.employee.department}</p>
                </div>
                <div className="text-right mr-4">
                  <p className="text-sm text-slate-600">Due: {format(parseISO(assignment.dueDate), 'MMM d, yyyy')}</p>
                  {assignment.completedDate && (
                    <p className="text-xs text-green-600">Completed: {format(parseISO(assignment.completedDate), 'MMM d, yyyy')}</p>
                  )}
                </div>

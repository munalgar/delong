...
const SeverityBadge = ({ severity }: { severity: Severity }) => {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 px-3 py-1.5 text-sm font-medium rounded-full ${styles[severity]}`}
    >
...
const StatusBadge = ({ status }: { status: IncidentStatus }) => {
  ...
  return (
    <span
      className={`px-3 py-1.5 text-sm font-medium rounded-full ${styles[status]}`}
    >
      {status}
    </span>
  );
...
export default function IncidentDetailPage() {
  ...
  const returnTo = searchParams.get("returnTo") || "/incidents";
  ...
  const workflowSteps = [
    { status: "Reported", completed: true },
    {
      status: "Under Review",
      completed:
        incident.status === "Under Review" || incident.status === "Closed",
    },
  ...
  return (
    <div className="p-8">
      ...
      >
        ...
        Back to {returnTo === "/dashboard" ? "Dashboard" : "Incidents"}
      </Link>
      ...
      <div className="bg-white rounded-xl shadow-sm p-6 mb-6">
        ...
        <div className="flex flex-wrap gap-6 text-sm mt-6">
          <div className="flex items-center gap-2 text-slate-600">
            ...
            <span>
              {format(parseISO(incident.dateTime), "MMMM d, yyyy h:mm a")}
            </span>
          </div>
          <div className="flex items-center gap-2 text-slate-600">
            ...
            <div className="p-6">
              ...
              incident.documents.length === 0 ? (
                <p className="text-slate-500 text-center py-4">
                  No documents or photos attached
                </p>
              ) : (

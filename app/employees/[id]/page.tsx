...
import { getEmployeeById, getTrainingById, getIncidentById } from '@/lib/mock-data';
import { format, parseISO, differenceInDays } from 'date-fns';
import { TrainingStatus, CertificationStatus, ComplianceStatus } from '@/lib/types';
...
const CertStatusBadge = ({ status }: { status: CertificationStatus }) => {
  const config = {
    ...
    'expiring-soon': { bg: 'bg-yellow-100', text: 'text-yellow-700', icon: Clock, label: 'Expiring Soon' },
...
const ComplianceBadge = ({ status }: { status: ComplianceStatus }) => {
  ...
  return (
    <span className={`inline-flex items-center gap-1 px-3 py-1.5 text-sm font-medium rounded-full ${bg} ${text}`}>
...
export default function EmployeeProfilePage() {
  ...
  const [trainingFilter, setTrainingFilter] = useState<'all' | 'assigned' | 'in-progress' | 'overdue'>('all');
  ...
  const filteredTraining = employee.trainingAssignments.filter(t => {
    ...
    if (trainingFilter === 'in-progress') return t.status === 'in-progress' || t.status === 'at-risk';
  ...
  const certStats = {
    ...
    expiring: employee.certifications.filter(c => c.status === 'expiring-soon').length,
    expired: employee.certifications.filter(c => c.status === 'expired').length
  };

  const trainingStats = {
    total: employee.trainingAssignments.length,
    complete: employee.trainingAssignments.filter(t => t.status === 'complete').length,
    inProgress: employee.trainingAssignments.filter(t => t.status === 'in-progress' || t.status === 'at-risk').length,
    overdue: employee.trainingAssignments.filter(t => t.status === 'overdue').length
  };

  return (
    <div className="p-8">
      {/* Back button */}
      <Link href="/employees" className="inline-flex items-center gap-2 text-slate-600 hover:text-slate-900 mb-6">
          ...
          <div className="flex-1">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
              ...
              <div className="flex items-center gap-2 text-slate-600">
                ...
                <span>Hired {format(parseISO(employee.hireDate), 'MMM d, yyyy')}</span>
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="bg-white rounded-xl shadow-sm">
            <div className="p-6 border-b border-slate-100">
              ...
              <div className="grid grid-cols-3 gap-2 text-center">
                <div className="p-2 bg-green-50 rounded-lg">
                  <p className="text-xl font-bold text-green-600">{certStats.valid}</p>
                  <p className="text-xs text-green-600">Valid</p>
                </div>
                <div className="p-2 bg-yellow-50 rounded-lg">
                  <p className="text-xl font-bold text-yellow-600">{certStats.expiring}</p>
                  <p className="text-xs text-yellow-600">Expiring</p>
                </div>
                <div className="p-2 bg-red-50 rounded-lg">
                  <p className="text-xl font-bold text-red-600">{certStats.expired}</p>
            ...
            <div className="p-4 space-y-3 max-h-96 overflow-y-auto">
              {employee.certifications.map(cert => (
                <div key={cert.id} className="p-3 border border-slate-200 rounded-lg">
                  <div className="flex items-start justify-between mb-2">
                    <h3 className="font-medium text-slate-900 text-sm">{cert.name}</h3>
                    <CertStatusBadge status={cert.status} />
                  </div>
                  <div className="text-xs text-slate-500 space-y-1">
                    <p>Issued: {format(parseISO(cert.issuedDate), 'MMM d, yyyy')}</p>
                    <p>Expires: {format(parseISO(cert.expirationDate), 'MMM d, yyyy')}</p>
                    {cert.status !== 'expired' && (
                      <p className={cert.status === 'expiring-soon' ? 'text-yellow-600 font-medium' : ''}>
                        {differenceInDays(parseISO(cert.expirationDate), new Date())} days remaining
                      </p>
                    )}
                  </div>
                </div>
          ...
          <div className="bg-white rounded-xl shadow-sm">
            <div className="p-6 border-b border-slate-100">
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-2">
                  ...
                  <h2 className="text-lg font-semibold text-slate-900">Training Overview</h2>
                </div>
              </div>
              
              <div className="grid grid-cols-4 gap-2 text-center mb-4">
                <div className="p-2 bg-slate-50 rounded-lg">
                  <p className="text-xl font-bold text-slate-700">{trainingStats.total}</p>
                  <p className="text-xs text-slate-500">Total</p>
                </div>
                <div className="p-2 bg-green-50 rounded-lg">
                  <p className="text-xl font-bold text-green-600">{trainingStats.complete}</p>
                  <p className="text-xs text-green-600">Complete</p>
                </div>
                <div className="p-2 bg-blue-50 rounded-lg">
                  <p className="text-xl font-bold text-blue-600">{trainingStats.inProgress}</p>
                  <p className="text-xs text-blue-600">In Progress</p>
                </div>
                <div className="p-2 bg-red-50 rounded-lg">
                  <p className="text-xl font-bold text-red-600">{trainingStats.overdue}</p>
              ...
              <div className="flex gap-2">
                {(['all', 'assigned', 'in-progress', 'overdue'] as const).map(filter => (
                  <button
                    key={filter}
            ...
            <div className="p-4 space-y-3 max-h-[500px] overflow-y-auto">
              {filteredTraining.length === 0 ? (
                <p className="text-center text-slate-500 py-8">No training assignments found</p>
              ) : (
                filteredTraining.map(assignment => {
                  ...
                  return (
                    <div key={assignment.id} className="border border-slate-200 rounded-lg overflow-hidden">
                      <button
                        onClick={() => setExpandedTraining(isExpanded ? null : assignment.id)}
                        className="w-full p-4 flex items-center justify-between hover:bg-slate-50 transition-colors"
                      >
                        <div className="flex-1 text-left">
                          <div className="flex items-center gap-3 mb-1">
                            <h3 className="font-medium text-slate-900">{training?.title}</h3>
                            <TrainingStatusBadge status={assignment.status} />
                          </div>
                          <p className="text-sm text-slate-500">Due: {format(parseISO(assignment.dueDate), 'MMM d, yyyy')}</p>
                        </div>
                        {isExpanded ? (
                        ...
                        <div className="px-4 pb-4 bg-slate-50 border-t border-slate-100">
                          <div className="pt-4 space-y-2 text-sm">
                            <div className="flex justify-between">
                              ...
                              <span className="text-slate-700">{format(parseISO(assignment.assignedDate), 'MMM d, yyyy')}</span>
                            </div>
                            <div className="flex justify-between">
                              <span className="text-slate-500">Due Date:</span>
                              <span className={`${assignment.status === 'overdue' ? 'text-red-600 font-medium' : 'text-slate-700'}`}>
                            ...
                            {assignment.completedDate && (
                              <div className="flex justify-between">
                                ...
                                <span className="text-green-600 font-medium">{format(parseISO(assignment.completedDate), 'MMM d, yyyy')}</span>
                              </div>
                            )}
                            <div className="flex justify-between">
                              <span className="text-slate-500">Duration:</span>
                              <span className="text-slate-700">{training?.duration}</span>
                            </div>
                            <div className="pt-2">
                              <Link
                ...
                return (
                  ...
                  >
                    ...
                    <p className="text-sm text-slate-500 mt-1">
                      {format(parseISO(incident.dateTime), 'MMM d, yyyy')} • {incident.location}
                    </p>
                  </Link>
                );
              })}

...
export default function EmployeesPage() {
  ...
  const filteredEmployees = useMemo(() => {
    return employees.filter((emp) => {
      ...
      const matchesStatus =
        !selectedStatus || emp.complianceStatus === selectedStatus;
    ...
    <div className="p-8">
      <div className="mb-8">
        ...
        <p className="text-slate-600 mt-1">
          Manage employee safety profiles and compliance
        </p>
      </div>
          ...
          <div className="px-4 pb-4 flex flex-wrap gap-4 border-t border-slate-100 pt-4">
            ...
            {(selectedDepartment || selectedStatus) && (
              <button
                ...
                className="self-end px-3 py-2 text-sm text-slate-900 hover:text-orange-600"
              >
                Clear filters
              </button>
            )}
            ...
            >
              ...
              <div className="hidden md:flex items-center gap-6 mr-4">
                ...
                <div className="text-center">
                  <p className="text-lg font-semibold text-slate-900">
                    {
                      employee.trainingAssignments.filter(
                        (t) => t.status === "complete"
                      ).length
                    }
                    /{employee.trainingAssignments.length}
                  </p>
                  <p className="text-xs text-slate-500">Training</p>
                </div>
                <div className="text-center">
                  <p className="text-lg font-semibold text-slate-900">
                    {employee.incidentIds.length}
                  </p>
